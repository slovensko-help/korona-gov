...
function prefillRcFormWithTestData() {
    ...
    $('#uc-phone-verify').val('+421919123456');
...
(function($) {
    ...
    $(document).ready(function() {
        ...
        function formData() {
            ...
            if (result['exception'] !== 'work-abroad') {
                result['work-abroad-address'] = '';
                result['work-abroad-company-name'] = '';
            }

            if (result['exception'] !== 'foreign-permanent-address') {
                result['foreign-permanent-address'] = '';
            }

            var exceptionMapping = {
                'none': null,
                'foreign-permanent-address': 1,
                'work-abroad': 2,
                'vaccinated': 3,
                'had-covid': 4
            }

            result['exception'] = exceptionMapping[result['exception']];
        ...
        function isFormValid() {
            ...
            if (step === 2) {
                ...
                // 'work-abroad'
                if (data['exception'] === 2) {
                    isValid &= toggleInputFormError('uc-work-abroad-address', '' !== data['work-abroad-address']);
                    isValid &= toggleInputFormError('uc-work-abroad-company-name', '' !== data['work-abroad-company-name']);
                }

                // 'foreign-permanent-address'
                if (data['exception'] === 1) {
                    isValid &= toggleInputFormError('uc-foreign-permanent-address', '' !== data['foreign-permanent-address']);
                }
        ...
        function changeFormStep(activeStep, isTriggeredByFormSubmit) {
            ...
            if (step === 2) {
                ...
                $('.js-back-button').show();
                $('.js-fill-form-with-test-data').hide();
                ...
                $('.js-countries').show();
                $('.js-fill-form-with-test-data').show();
        ...
        $('body')
            ...
            .on('change', '.js-uc-exception', function() {
                var $checkedRadio = $('[name=exception]:checked');
                var $foreignPermamentAddressFields = $('.js-uc-foreign-permanent-address-fields');
                var $workAbroadFields = $('.js-uc-work-abroad-fields');
                var $haveException = $('.js-uc-have-exception');

                $foreignPermamentAddressFields.hide();
                $workAbroadFields.hide();

                if ('foreign-permanent-address' === $checkedRadio.val()) {
                    $foreignPermamentAddressFields.show();
                }
                else if ('work-abroad' === $checkedRadio.val()) {
                    $workAbroadFields.show();
                }

                if ('none' === $checkedRadio.val()) {
                    $haveException.hide();
                }
                else {
                    $haveException.show();
                ...
                if (step === 2) {
                    ...
                    $.each(data.people, function (i, person) {
                        var personData = {
                            ...
                            "foreign_address": data['foreign-permanent-address'],
                            "employer_name": data['work-abroad-company-name'],
                            "employer_address": data['work-abroad-address'],
                        }
        ...
        if (isInTestMode) {
            $form.prepend('<div style="margin-bottom: 100px;" class="js-fill-form-with-test-data">\n' +
