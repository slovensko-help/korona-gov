if (window.location.href.indexOf('test=test') > -1)
(function($) {
    ...
    var rcCountryAutocompleteInit = function (name) {
    ...
    $(document).ready(function() {
        ...
        function lpad(string, padstring) {
        ...
        $('body')
            .on('click', '.js-uc-add-country', function(event) {
                ...
                rcCountryAutocompleteInit(countryId);
        ...
        $('#fill-form-with-test-data').on('click', function(event) {
            event.preventDefault();
            ...
            if (isFormValid()) {
                ...
                $.each(data.people, function(i, person) {
                    people.push({
                        ...
                        "dEntry_from_abroad_planned_at": data['arrival-year'] + '-' + lpad(data['arrival-month'], '00') + '-' + lpad(data['arrival-day'], '00'),
                        ...
                        "dDateOfBirth": person['dob-year'] + '-' + lpad(person['dob-month'], '00') + '-' + lpad(person['dob-day'], '00'),
                ...
                $.ajax({
                    type: 'POST',
                    url: 'https://t.mojeezdravie.sk/api/v1/risk/new-pass',
                    data: JSON.stringify({
                        people: people
                    }),
                    error: function() {
                    ...
                    success: function(response) {
                        if (!response.payload || !response.payload.vCovid19Pass) {
                            ...
                            if (response.errors &&
                                ...
                                if (response.errors[0].description.indexOf('rodné číslo existuje') > -1) {
                                    $('#uc-slovak-id-registered').show();
                                } else if (response.errors[0].description.indexOf('zahraničný identifikátor existuje') > -1) {
